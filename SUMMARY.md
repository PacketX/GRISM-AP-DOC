# Table of contents

## GET STARTED

* [GRISM AP documentation](README.md)
* [Ethernet Port Link LED](get-started/eth_port_link_led.md)

## Console Port

* [Console Port](console/README.md)
  * [Global](console/global.md)
  * [set](console/set.md)
  * [reset](console/reset.md)


## XML Script

* [XML Elements](xml-script/elements/README.md)
  * [\<run>](xml-script/elements/run.md)
  * [\<chain>](xml-script/elements/chain.md)
  * [\<filer>](xml-script/elements/filter/README.md)
    * [\<find>](xml-script/elements/filter/find.md)

## XML Case Studies

* [Case Sample](xml-case-studies/README.md)
  * [Aggregating traffic](xml-case-studies/aggregating_traffic.md)
  * [GTP Filtering](xml-case-studies/filter_gtp_traffic.md)
  * [GTP Filtering by Port](xml-case-studies/filter_gtp_traffic_by_port.md)
  * [Test Case](xml-case-studies/test-case/README.md)
    * [Traffic Aggregation](xml-case-studies/test-case/traffic-aggregation.md)
    * [Traffic Distribution](xml-case-studies/test-case/traffic-distribution.md)
    * [GTP-U Outer Encapsulation Stripping](xml-case-studies/test-case/gtpu_outer_encapsulation_stripping.md)
    * [GTP-U Inner Filtering](xml-case-studies/test-case/gtpu_inner_filtering.md)
    * [L2 - L4 Composite Filtering](xml-case-studies/test-case/l2_l4_composite_filtering.md)
    * [GTP-C and GTP-U Filtering](xml-case-studies/test-case/gtp_c_and_gtp_u_filtering.md)
    * [Real Task 1](xml-case-studies/test-case/real_task_1.md)

