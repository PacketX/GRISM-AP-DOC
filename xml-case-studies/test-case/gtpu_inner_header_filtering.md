# GTP-U Inner Header Filtering

This case study shows how to filter traffic using **inner (tunneled) headers** inside GTP-U, and forward matched packets to an output port.

## Flow

![GTP-U Inner Header Filtering Flow](/.gitbook/assets/gtpu-inner-header-filtering.svg)
...
- **Result**: DNS traffic to/from `8.8.8.8` is forwarded to `S2.1` based on inner headers.

## Operational Notes

- This case assumes packets contain an inner IP/UDP header (for example, traffic carried inside GTP-U).
- If you need to ensure only GTP-U packets are evaluated, combine this with a GTP-U filter (for example, `gtp.data`) before applying inner header conditions.
